...
from solace_monitoring_003.solace_per_bridge_stats import SolacePerBridgeStats
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        for env_conf_file in os.listdir(envs_dir):
            if env_conf_file.endswith(conf_extention):
                solace_per_bridge_stats = SolacePerBridgeStats(
                    common_conf_file,
                    os.path.join(envs_dir, env_conf_file),
                    metrics_list_file,
                )
                threads.append(
                    executor.submit(solace_per_bridge_stats.run, env_conf_file)
                )
